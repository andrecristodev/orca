...
import type { StatusBarToggleEntry } from './appearance-status-bar-search-entries'

// Why: split out from the provider usage toggles so each file stays under the
// 300-line max-lines cap (see AGENTS.md). These are the non-usage status-bar
// items (remote hosts, resource manager, ports).
export function buildSystemStatusBarToggleEntries(): readonly StatusBarToggleEntry[] {
  return [
    {
      id: 'ssh',
